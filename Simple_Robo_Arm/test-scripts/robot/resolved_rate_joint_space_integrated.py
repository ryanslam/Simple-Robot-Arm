...
def track_trajectory(control_func:function, start:tuple[float], goal:tuple[float]):
    ...
    global arm
    arm.set_joint_pose(goal, True)
    dt = arm._traj.t[1] - arm._traj.t[0]
    while arm.has_trajectory:
        if arm._traj.done: break
        loop_start = time.monotonic()
        t, q, qd = arm._traj.step()
        # control_func(q, qd)
        arm.control_manager.active_controller.set_target(q, qd)
        arm.read_motor_states()
        qd = arm.control_manager.active_controller.control_law(arm.Q)
        arm.set_joint_velocities(qd)
